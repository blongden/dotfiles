...
| `gtk` | `~/.config/gtk-{3,4}.0/{settings.ini,gtk.css}` | GTK3 + GTK4/libadwaita: dark Adwaita base + a tinty-managed `@define-color` recolour block (`fanout.sh`). GTK3 live-reloads; GTK4 apps need a restart. |
| `qt` | `~/.config/kdeglobals`, `~/.config/environment.d/20-qt.conf` | Qt/KF6 apps (Dolphin, Ark, Okular): tinty-managed `kdeglobals` colour scheme, no Breeze/Plasma deps. `QT_QPA_PLATFORMTHEME=gtk3` routes plain-Qt apps (VLC) through the GTK theme. Needs `qt5-gtk-platformtheme` / `qt6-gtk-platformtheme`. |
